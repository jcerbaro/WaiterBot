...
int fuzzy_control::definevars(linguisticvariable &input1,linguisticvariable &input2,
                              linguisticvariable &input3,linguisticvariable &input4,
                              linguisticvariable &input5,linguisticvariable &output)
{
numinputvars = 3;
lingvarinput1 = input1;
lingvarinput2 = input2;
lingvarinput3 = input3;
lingvarinput4 = input4;
lingvarinput5 = input5;
lingvaroutput = output;
return(1);
}
...
int fuzzy_control::insert_rule(const char *setofinplingvar1,const char *setofinplingvar2,
                               const char *setofinplingvar3,const char *setofinplingvar4,
                               const char *setofinplingvar5,const char *setofoutlingvar)
{
  if(numrules<MAXRULES)
  {
         rulebase[numrules] = new rule;
         rulebase[numrules]->definerule(&lingvarinput1.getcat(setofinplingvar1),
                                        &lingvarinput2.getcat(setofinplingvar2),
                                        &lingvarinput3.getcat(setofinplingvar3),
                                        &lingvarinput4.getcat(setofinplingvar4),
                                        &lingvarinput5.getcat(setofinplingvar5),
                                        &lingvaroutput.getcat(setofoutlingvar));
         numrules++;
         return(0);
  }
...
float fuzzy_control::make_inference(float inputval1,float inputval2,float inputval3,float inputval4,float inputval5)
{
for(int i=0;i<numrules;i++)
   {
    rulebase[i]->clearinputcategoryactivations();
...
for(int i=0;i<numrules;i++)
   {
     activationofrule[i]=rulebase[i]->evaluaterule(inputval1, inputval2, inputval3, inputval4, inputval5);
#ifdef DEBUG
     cout << "Activ of Rule " << i << "= ";
     cout << activationofrule[i] << "\n";
#endif
